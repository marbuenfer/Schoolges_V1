...
<!-- EN CONSTRUCCIÓN  -->
	...
	<div class="container mt-5">
		<h2 class="mb-4 text-center">MIS ACTIVIDADES</h2>
		<div class="row justify-content-center">
			...
			<div class="col-md-6">
				...
				%>
				<input type="hidden" id="idAluSeleccionado"
					value="<%=userObj.getIdUsu()%>">

				<div class="form-group">
					<label for="estudio" class="text-center d-block">Selecciona
						un Estudio:</label> <select name="idEst" id="idEst_id"
						class="form-control">
						<%-- Aquí puedes recuperar las matrículas del alumno utilizando userObj.idAlu --%>
						<%-- Asegúrate de que userObj tenga una propiedad idAlu que contenga el ID del alumno --%>
						<%-- Luego, utiliza ese ID para obtener las matrículas del alumno --%>
						<%
						DaoMatricula daoMatricula = new DaoMatricula(DbConexion.getConn());
						List<Matricula> listaMatriculas = daoMatricula.getEstudiosMatriculadosByAlumnoRolAlumnoDistinct(userObj.getIdUsu());
						for (Matricula mat : listaMatriculas) {
						%>
						<option value="<%=mat.getIdEst()%>"><%=mat.getIdEst()%> -
							<%=mat.getNomEst()%></option>
						<%
						}
						%>
					</select>
					...
					<table class="table table-striped table-responsive"
						...
						<thead>
							<tr>
								...
								<th class="col-md-2">Enunciado Act.</th>
								<th>Nota</th>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<!-- 					<div class="form-group"> -->
					<!-- 						<label for="idMatActualizar">ID Materia:</label> <input type="text" -->
					<!-- 							class="form-control" id="idMatActualizar_id" readonly> -->
					<!-- 					</div> -->
					<!-- 					<div class="form-group"> -->
    ...
    */
        $(document).ready(function() {
            ...
            console.log("estoy en document.ready primerIdEstudio seleccionado---" + primerIdEstudio);
        ...
        $('#idEst_id').change(function() {
            ...
            var idAluSeleccionado =  $('#idAluSeleccionado').val();
            
            console.log("Estoy en evento onchange de idEst---" );

            console.log("idEstudioSeleccionado---" + idEstudioSeleccionado);
            console.log("idAluSeleccionado---" + idAluSeleccionado);
        ...
        function obtenerMateriasPorEstudio(idEst) {
            ...
            var servletUrl = contextPath + "/SvObtenerMatriculacionesByMateriaAlumno";
            console.log("idEst  antes de pasarlo a servlet---" + idEst);
            
            var idAlu  =  $('#idAluSeleccionado').val();
            var url = servletUrl + "?idEst=" + idEst;
            url += "&idAlu=" + idAlu;

            console.log("var url en funcion obtenerMAteriasPor Estudio---" + url);
        ...
        /**
         * Asocia una función para cargar las actividades por materia y alumno cuando se cambia la opción seleccionada en el elemento con ID ''.
         * La función obtiene el ID de la materia seleccionada y el id alumno y llama a la función 'cargarActividadesMateria' con estos IDs para cargar las activiadades correspondientes.
         */
      $('#idMat_id').change(function() {
      var idMatSeleccionada = $(this).val();
      var idAluSeleccionado = $('#idAluSeleccionado').val();
      cargarActividadesMateria(idMatSeleccionada, idAluSeleccionado);
  });
         ...
         */
         ...
         function cargarActividadesMateria(idMat, idAlu) {
            // Construir la URL con los parámetros de consulta
            var url = contextPath + "/SvObtenerActividadesByAlumno";
     	    url += "?idAlu=" + idAlu; // Añadir el parámetro tipo
    	    url += "&idMat=" + idMat; 
            $
                    .ajax({
     ...
     // Listener para el botón "Guardar" del modal
        $('#botonModalActualizar').click(function() {
			...
			actividadEditada.notaAct = nuevaNotaAct;
 ...
 */
 $('#botonNuevaActividad').click(function() {
	    ...
	    url += "&idMat=" + idMat; // Corregir la concatenación utilizando el operador &
...
$('#botonModalInsertar').click(function() {
   ...
   // var idAct =  $('#idAct_id').val();
    var idMat =  $('#idMat_id').val();
    var obsMatEst = $('#obsActMatInsertar_id').val();
 ...
 $('#botonModalInsertarCerrar').click(function() {
   ...
   */
   function editarActividad(button) {
	    var obsAct = $(button).closest('tr').find('td:eq(5)').text(); // Obtener obsAct de la sexta celda de la fila
